# Base scenario validation — candidate 0.2.0
...
These checks are **representative, not 80 scenario-specific replays**. The per-base executed count
therefore remains zero. Live evidence is kept separate from generated JSON; generated state rows
are explicitly marked EXPECTED_FIXTURE.

The public summary is `experiments/configs/m2-fork-validation.json`. Actual Aave position amounts
