...
대회가 요구하는 팀 식별 정보와 제출 링크는 저장소 밖에서 두 팀원이 직접 관리한다. 이 원고에는 실제 값, placeholder, 외부 증빙 위치를 기록하지 않는다. 최종 제출용 Notion에만 필수 메타데이터를 입력하며, 저장소의 원고와 자동 생성 산출물에는 포함하지 않는다.
