# IntentLock: Agent Wallet의 멀티 툴 실행을 위한 누적 경제 의도 계약
...
2. 본 연구의 시스템 비교는 80개 기본 의도에서 만든 400개 trace에 대한 offline counterfactual replay다. 기본 의도 80개의 고정 포크 실행 검증과 40개의 scripted signer-boundary 실험은 별도의 증거로 보고한다.
3. 결과의 해석 단위는 지원된 계약·프로토콜·정적 사례다. 자연어 추출기의 일반 정확도, 실제 공격 LLM에 대한 강건성, MetaMask 운영 서비스의 취약점 또는 배포 성능을 이 결과에서 추론하지 않는다.
...
완전한 bytecode 분석기는 아니다. 주 offline 실험의 입력 효과는 versioned trace에 작성된 predicted effects이며, 중첩 효과 비교도 이 표현 위에서 이루어진다. 따라서 이 실험의 shallow-decoder 차이는 중첩 경제 효과의 관측이 얼마나 필요한지를 보여주는 것이지, 임의의 악성 calldata에 대한 실제 decoder recall을 측정하는 것이 아니다. 실제 calldata와 receipt의 연결은 고정 포크 기본 사례와 별도 signer-boundary 사례에서 검사한다.
...
기본 의도마다 정상 trace, scope 공격, budget 공격, composition 공격과 비적대적 drift를 하나씩 적용해 400개 사례를 구성한다. 변이가 적용되지 않는 workflow에서는 등록된 같은 범주의 다음 연산자를 사용하고, 단순 no-op을 공격으로 세지 않는다. PRE_SIGN에서 알 수 없는 post-state-only 변화는 관측 단계를 표시한다. 이름에 HIDDEN_TEST가 남아 있는 분할도 개발 중 저자에게 노출되었으므로 author-exposed evaluation split이다.
...
주 지표인 counterfactual unsafe-authorization rate는 평가 episode 전체를 분모로 두고, 승인된 authored trace의 exact oracle이 계약 위반을 나타내는 episode를 분자로 둔다. 이름은 다음 사실을 보존한다. 여기서 승인과 경제 결과는 offline 입력에 대한 반사실적 판단이며, 모든 trace를 실제로 서명·실행한 관측이 아니다. 실제 transaction UER 또는 실제 공격 성공률과 동일하지 않다.
...
구성요소 실험은 400개 사례에 8개 arm을 적용한다. full reference, semantic-only, symbolic-only, hybrid conjunction, accepted-effect history 제거, shallow effect 관측, post-state verifier 제거와 always-confirm 정책을 구분한다. 앞의 stage comparison은 이미 확인된 계약에서 시작하며 여러 단계가 함께 달라질 수 있으므로 자연어 compiler의 인과적 기여로 해석하지 않는다. always-confirm도 별도의 상호작용 정책이다. 한 요소만 바꾸는 비교는 설정 diff로 확인되는 history·중첩 효과·post-state 세 요인이다.
...
오류 그림의 분류는 원시 outcome·reason에 따른 기술 통계다. specification error, extraction error, enforcement error, protocol semantics와 transport failure의 원인을 모두 독립적으로 실증한 비율은 아니다. 특히 authored reference 결함은 system의 unsafe authorization으로 재분류하지 않는다.
...
accepted-effect history 제거가 cumulative case에서 만든 변화는 이전 호출의 효과를 기억하는 역할에 관한 결과다. shallow 관측과 post-state 제거는 각각 입력에서 가려진 효과와 발견 시점의 차이를 분리한다. post-state verifier를 제거해 사후 발견이 줄어도 이미 일어난 위반의 사전 방지 효과라고 해석하지 않는다. semantic-only·hybrid의 변화는 고정 LLM 결과와 symbolic 결과를 조합한 stage comparison이며 자연어 이해의 완전성을 측정하지 않는다.
...
연구 개발·라벨 명세·검토는 단일 저자가 책임지는 방식이며 AI를 코드 작성, 자료 대조, 반례 탐색과 원고 점검에 활용한다. AI 보조 검토를 두 번째 독립 인간 reviewer로 계산하지 않는다. 인간 간 일치도, double-blind 평가나 독립 인간 검수가 수행되었다는 주장은 하지 않는다. 이 절차는 작업과 검증의 추적 가능성을 높이지만 저자와 도구가 공유한 오류 가능성을 없애지는 못한다.
