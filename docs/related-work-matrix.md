...
| [CaMeL](https://arxiv.org/abs/2503.18813) v2                    | trusted query에서 control/data flow를 추출하고 untrusted data가 흐름을 바꾸지 못하게 격리, capability policy | AgentDojo task의 77%를 provable security와 함께 해결했다고 보고                   | control/data-flow 보호와 달리 허용된 gadget의 cumulative economic outcome을 직접 bound하지 않음                                   | **Tier B**: 환경 이식 비용 확인 후 포함                  |
